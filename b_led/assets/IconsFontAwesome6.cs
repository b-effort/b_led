...
namespace IconFonts
{
    public static partial class FontAwesome6
    {
        static nint glyphRanges;
        static byte[]? fontBuffer;

        static unsafe void Init() {
            ushort[] ranges = { IconMin, IconMax, 0 };

            fixed (ushort* rangesPtr = &ranges[0]) {
                glyphRanges = Marshal.AllocHGlobal(6);
                Buffer.MemoryCopy(rangesPtr, glyphRanges.ToPointer(), 6, 6);
            }

            fontBuffer = Convert.FromBase64String(fontData);
        }

        public static unsafe void Load(ImGuiIOPtr io, int size_pt) {
            if (fontBuffer is null)
                Init();
            ...
            var config = new ImFontConfig {
                ...
                OversampleH = 2,
                OversampleV = 1,
                GlyphRanges = (ushort*)glyphRanges.ToPointer(),
            };

            fixed (byte* buffer = fontBuffer) {
                io.Fonts.AddFontFromMemoryTTF(new IntPtr(buffer), fontBuffer!.Length, size_pt, &config);
            }
        }
